...
cd %s""" % self.oBatchSettings.batchScriptDirectory
    ...
    #Not relevant as this info is already in the settings file
#     	newLstExp = []; nb_deja = 0
#     	for plate, pos in lstExperiments:
#     	    dataFolder =self.oBatchSettings.baseOutDir
#             if 'hdf5' in os.listdir(os.path.join(dataFolder, plate)) and pos+'.hdf5' in os.listdir(os.path.join(dataFolder, plate, 'hdf5')):
#                 print plate, pos, ": deja fait"
#                 nb_deja+=1
#             else:
#     		    newLstExp.append((plate, pos))
        ...
        print 'found : %i experiments' % len(lstExperiments)
        #print 'already done : %i experiments' % nb_deja
        print 'job size (settings file) : %i' % jobSize
        print 'number of jobs : %i' % nb_jobs
...
%s %s --settings %s --input "%s" --output "%s" --position_list %s """
        ...
        #from util import path_command, pbsErrDir, pbsOutDir
        path_command = self.oBatchSettings.path_command
        pbsErrDir = self.oBatchSettings.pbsErrDir
        pbsOutDir = self.oBatchSettings.pbsOutDir
...
#         else:#if self.oBatchSettings.plates is not None:
#             plates = self.oBatchSettings.plates
# #	else:
# #	    plates = os.listdir(os.path.join(self.oBatchSettings.baseInDir))
# #	    np.random.shuffle(plates); plates=plates[:100]
# 
#         lstExperiments = []
#         for plate in plates:
...
#    f=open(options.position_file, 'r')
#    lstExperiments = pickle.load(f); f.close()
